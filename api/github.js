...
export default async function handler(req, res) {
  ...
  // ── ACTION: list (password ছাড়াই — site boot এ দরকার) ──
  if (action === 'list') {
    try {
      const r = await fetch(`${BASE}/playlist`, { headers: HEADERS });
  ...
  // ── Password Check (verify/upload/delete এর জন্য) ───────
  ...
  // ── ACTION: upload (create or update a file in playlist/) ─
  if (action === 'upload') {
    if (!filename || content === undefined) return res.status(400).json({ error: 'filename and content required' });
    const safeName = filename.replace(/[^a-zA-Z0-9._-]/g, '_');
    const filePath = `playlist/${safeName}`;
    try {
      let sha;
      const check = await fetch(`${BASE}/${filePath}`, { headers: HEADERS });
      if (check.ok) {
        const existing = await check.json();
        sha = existing.sha;
      }
      const body = {
        message: `MNLive Admin: update ${filePath}`,
        content: btoa(unescape(encodeURIComponent(content))),
        ...(sha ? { sha } : {}),
      };
      const r = await fetch(`${BASE}/${filePath}`, {
  ...
  // ── ACTION: delete (from playlist/ folder) ──────────────
  if (action === 'delete') {
    ...
    try {
      ...
      const r = await fetch(`${BASE}/${filePath}`, {
        ...
        body: JSON.stringify({ message: `MNLive Admin: delete ${filePath}`, sha: existing.sha }),
      });
